...
#define ITERS 800000
#define type int
static snmalloc::BBQ<type, 8, 15> test_q{};
#ifdef DEBUG
#  define log snmalloc::BBQ<type, 8, 15>::BBQLog("bbq.log")
#else
class NullLogger
{
public:
...
using status = snmalloc::BBQ<type, 8, 15>::Queuestatus;

#ifdef TEST_BBQ_CHECK_ENTRIES
static std::mutex map_mutex;
static std::unordered_map<type, int> enq_item_count;
static std::unordered_map<type, int> deq_item_count;
#endif
...
void writer(int id)
{
  ...
  {
    status st{};
    while ((st = test_q.enq(static_cast<type>(i * id))))
    {
      switch (st)
      {
        case status::BUSY:
...
int main()
{
  ...
  log << "BBQ: finish writing and reading with throughput = "
      ...
      << "[logging enabled(turn off to get real throughput)]\n";
#else
      << '\n';
#endif
